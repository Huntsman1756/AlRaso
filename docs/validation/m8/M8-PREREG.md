...
| 2026-09-18 | M8-F tooling | DONE (runner) — `tooling/m8_holdout_run.py`: recomputa commitment sobre casos sellados (abort en mismatch), ejecuta contra resolver congelado vía manifiesto, mapeo de clases preregistrado, `protocol_failures` para boundary limpio; §6 M8-HOLDOUT documenta ejecución. Ejecución real: **bloqueada por custodio** (casos sellados + corpus publicado post-adjudicación) |
| 2026-09-18 | M8-G smoke | DONE (camino CLI) — `docs/validation/m8/M8G-SMOKE.md`: 6 puntos reales sobre capas IDEM hash-pinned + 4 corpus candidatos; scopes correctos, hole→UNKNOWN, vértice real→`BOUNDARY_AMBIGUOUS`, todo fail-closed `UNDETERMINED` pre-adjudicación (correcto). CLI `resolve --layers-manifest` + `load-corpus` añadidos; `CONDITIONAL` renderizado en `ui_texto`/`legal.js` (nunca "sí" desnudo). Webapp-Madrid: PENDING |
