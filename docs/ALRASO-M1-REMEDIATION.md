...
| Gate | Resultado |
|---|---|
| Suite hermetica (host, Python 3.11.15, sin red ni motor externo) | **276 passed, 5 skipped** (los 5 requieren binario Axiom). Perfil de dependencias, dicho con precision: el **paquete** es stdlib-only; la suite usa ademas el extra opcional `alraso[axiom]` (PyYAML) para los 20 tests de proyeccion RuleSpec, que **se saltan con motivo explicito** si PyYAML no esta instalado (256 passed, 6 skipped). CI verifica los dos perfiles |
| Suite completa con Axiom real (Docker, python:3.12-slim, `run-docker.ps1`) | **281 passed, 0 skipped** |
...
- `internal/MILESTONE-1.md` es evidencia historica: su `M1_STATUS` y sus contadores NO
