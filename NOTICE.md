...
| DEM oficial para auto-elevación (`tooling/dem_prep.py` → GeoTIFF local + `webapp/data/dem/picos_mdt.meta.json`) — **NO cometido en git** | IGN/CNIG (Modelo Digital del Terreno / MDE; WCS `servicios.idee.es/wcs-inspire/mdt`, `MDT025_50N`, o COG) | 2026-09-06 | sha256 del tile preparado en el metadata local (`source_sha256`); sin tile preparado, `source_sha256=""` → runtime fail-closed | Reuse terms `NOT_VERIFIED` (claim open data CC BY 4.0; página primaria de términos no recuperable desde el sandbox). **No se redistribuye el raster**; solo metadata de provenance. Atribución: © IGN/CNIG |
