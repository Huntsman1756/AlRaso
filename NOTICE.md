...
| rasterio (`>=1.3,<2`) | BSD-3-Clause (con avisos GDAL/Rasterio del proyecto upstream) | Solo extra `alraso[dem]`: lectura de GeoTIFF/COG y muestreo de elevación en `webapp/dem.py`. El core `alraso` sigue **stdlib-only**; sin rasterio, `webapp/dem.py` falla cerrado (`DEM_EVIDENCE_INCOMPLETE`) | **No** (dependencia declarada en extra, no empaquetada) |
...
| BDDAE/INSPIRE — Limites municipales, provinciales y autonómicos (GML) | IGN/CNIG, Centro de Descargas: `https://centrodedescargas.cnig.es/CentroDescargas/limites-municipales-provinciales-autonomicos` → `docs/lineas_limite_gml.zip` (NO cometido en git) | 2026-08-10T13:04:13Z (dump timestamp) | `5bd73c530af995c05da8d9ff4e3d293f62d0dc91c5e48ee773fe881716c108a6` | **CC-BY-4.0** (Orden FOM/2807/2015 framework). CRS: EPSG:4258. Product scale: 1:25000. Official uncertainty: ~40 m for provisional tramos. **No se redistribuye el dump completo**; solo se derivan anillos mínimos de sector (park ∩ CCAA) + zona de incertidumbre (100m buffer de bordes compartidos) en el fixture. |
| Geometría del fixture Phase B Picos (`alraso/resources/fixture_picos.json`): límite PN (OAPN WFS, reproyectado a 4326, simplificado a 177 puntos) + sectores CCAA (BDDAE/CNIG oficial, intersectado con límite OAPN) + zona de incertidumbre (100m buffer de bordes CCAA compartidos) | OAPN-SIGRED (`view_red_oapn_limite_pn`, EPSG:25830) + BDDAE/CNIG (EPSG:4258) | 2026-09-07 | `47acbd3d1181cc19070424ea00d891c576b5fb85a8b40d1887133a7420edaa57` (BDDAE/CNIG official fixture + NORM_VALIDITY microfix 2026-09-08) | **Derived artifact**: geometría mínima derivada del dump oficial BDDAE + OAPN. GISCO NUTS2 geometry is historical reference only (kept in source_documents for provenance). Guard 100m basado en ~40 m de incertidumbre oficial documentada por IGN/CNIG. |
| Geometría mínima del fixture Phase B Picos (`alraso/resources/fixture_picos.json`): límite PN (OAPN WFS, reproyectado a 4326, simplificado a 177 puntos) + sectores CCAA (intersección del PN con GISCO NUTS2 ES12/ES13/ES41, simplificado) | OAPN-SIGRED (`view_red_oapn_limite_pn`, EPSG:25830) + Eurostat GISCO (NUTS2 01M 2024, 4326) | 2026-09-06 | `1bc51f25…a3ef` (digest de `fixture_picos.json`) | Justificación de reutilización mínima (análoga a la excepción del anillo ZUM Góriz, NOTICE §4): se redistribuye **solo** la geometría necesaria para el caso de determinación por coordenadas; el resto de vectores completos permanece fuera del repo (solo digests). OAPN/GISCO términos de reutilización `NOT_VERIFIED`; puntos a <1 km de frontera CCAA exigen re-verificación IDE (`M2A` block) |
| DEM oficial para auto-elevación (`tooling/dem_prep.py` → GeoTIFF local + `webapp/data/dem/picos_mdt.meta.json`) — **NO cometido en git** | IGN/CNIG — Modelo Digital del Terreno MDT25 vía WCS 2.0.1 `https://servicios.idee.es/wcs-inspire/mdt`, cobertura `Elevacion25830_25` (nativeFormat COG, EPSG:25830, malla 25 m; verificada por GetCapabilities/DescribeCoverage; **`MDT025_50N` NO existe** en el servicio). Descarga con **un `SUBSET` por dimensión** (`subset=x(...)&subset=y(...)`), formato `image/tiff` | 2026-09-06 | `source_sha256` del tile preparado en el metadata local; sin tile válido, `source_sha256=""` y `DEM_PREPARED=NO` → runtime fail-closed (`DEM_EVIDENCE_INCOMPLETE`). Gate real ejecutado localmente: sha256 `e8bcad011706b67d866d7baff33b5385380d4e7f382c5e1aa637a2a3797feac7`, CRS `EPSG:25830`, resolución 25 m, muestra P1 = 1510 m | **Reuse terms `VERIFIED`**: el propio GetCapabilities del WCS declara `AccessConstraints=CC BY 4.0 scne.es` y `Fees="No se aplican condiciones"` (evidencia primaria). Fuentes: [licencia/descargas IGN](https://www.ign.es/web/ign/portal/servicios/centro-de-descargas), [política de datos IGN](https://www.ign.es/web/ign/portal/ide-ign/descarga-de-datos) y [catálogo WCS MDT](https://servicios.idee.es/wcs-inspire/mdt?service=WCS&request=GetCapabilities&version=2.0.1). **No se redistribuye el raster**; solo metadata de provenance. Atribución: © Instituto Geográfico Nacional (IGN) / CNIG. `vertical_datum=orthometric`, `vertical_reference_detail=NOT_VERIFIED` |
...
| Polígonos de áreas protegidas (Ordesa, Picos) — OSM `boundary=protected_area` para contexto visual | OpenStreetMap vía Overpass (`https://overpass-api.de/api/interpreter`, relation/10036292, relation/2401595) → `webapp/protected_areas.json` | 2026-09-11 | `dd7729dc…` (Ordesa), `6b148edf…` (Picos) | **ODbL-1.0** — © contribuidores de OpenStreetMap. Redistribución aceptada per `pois.json` precedent. **Referencia cartográfica visual — NO delimitación legal oficial.** Cross-check digest con OAPN SIGRED: `NO (digest-only)`, no se redistribuye geometría oficial. |
| Cross-check oficial OAPN (digest only, NO redistribuido) | OAPN/MITECO SIGRED, WFS `view_red_oapn_limite_pn` (EPSG:25830) → `tooling/m81_official_crosscheck_results.json` | 2026-09-11 | `9e2099da…` (OAPN WFS response) | **NO (digest-only)** — Únicamente se almacenan digests SHA-256 y áreas en hectáreas. Nunca se redistribuyen coordenadas oficiales. Términos de reutilización `NOT_VERIFIED`. |
...
THIRD_PARTY_DATA_DISTRIBUTED=official_gis_samples + osm_poi_points (ODbL) + picos_phaseb_minimal_geometry (see section 4)
DATA_REUSE_TERMS_VERIFICATION=NOT_VERIFIED (M1.1 task; OSM/OAPN/GISCO still unverified)
DEM_REUSE_TERMS=VERIFIED (IGN/CNIG MDT25, CC-BY-4.0, see section 4)
DEM_LICENSE=CC-BY-4.0
DEM_VERTICAL_REFERENCE=NOT_VERIFIED (vertical_datum=orthometric)
...
PICOS_OFFICIAL_BOUNDARY_SOURCE=IGN/CNIG BDDAE/INSPIRE (dump 2026-08-10T13:04:13Z)
PICOS_BOUNDARY_GUARD_M=100
PICOS_BOUNDARY_GUARD_BASIS=VERIFIED_OFFICIAL_DOC (~40m CNIG official uncertainty, 2.5x conservative)
GISCO_RUNTIME_JURISDICTION=REMOVED (BDDAE/CNIG official replaces GISCO NUTS2 at runtime)
DEM_REUSE_TERMS=VERIFIED (IGN/CNIG MDT25, CC-BY-4.0, see section 4)
DEM_LICENSE=CC-BY-4.0
